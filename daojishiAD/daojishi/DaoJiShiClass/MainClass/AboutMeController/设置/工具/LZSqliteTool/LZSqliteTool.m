...
+ (FMDatabase*)LZDefaultDataBase {
    ...
    if (LZ_dbPath == nil) {
        [self LZCreateSqliteWithName:LZSqliteDataTableName];
    }
...
//更新数据表
+ (void)BCUpdateTable:(NSString*)tableName model:(LZDataModel*)model {
    FMDatabase *LZ_db = [self LZDefaultDataBase];
    if (![LZ_db open]) {
        [LZ_db close];
        return;
...
//        NSString *update = [NSString stringWithFormat:@"INSERT INTO '%@' (identifier,groupName,nickName,userName,password,urlString,dsc,groupID,email,titleString,contentString,colorString,pcmData) VALUES ('%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@','%@')",tableName,model.identifier,model.groupName,model.nickName,userName,psw,model.urlString,dsc,model.groupID,email,model.titleString,model.contentString,model.colorString,model.pcmData];
